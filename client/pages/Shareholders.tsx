...
export default function Shareholders() {
  ...
  const { data: dmat } = useQuery<DmatAccount[]>({ queryKey: ["dmat", "all"], queryFn: async () => (await api.get<Paginated<DmatAccount>>("/dmat", { params: { page: 1, limit: 1000 } })).data.data });
  const { data, isFetching } = useQuery<Paginated<Shareholder>>({ queryKey: ["shareholders", page], queryFn: async () => (await api.get<Paginated<Shareholder>>("/shareholders", { params: { page, limit: 10 } })).data });

  const createMutation = useMutation({
    mutationFn: async (payload: Omit<Shareholder, "_id">) => (await api.post<Shareholder>("/shareholders", payload)).data,
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["shareholders"] }); setOpen(false); setForm(empty); },
  });
  const updateMutation = useMutation({
    mutationFn: async (payload: Shareholder) => (await api.put<Shareholder>(`/shareholders/${payload._id}` , { ...payload, linkedDmatAccount: (payload.linkedDmatAccount as any)?._id || payload.linkedDmatAccount })).data,
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["shareholders"] }); setEditing(null); },
  });
  ...
  const columns = useMemo(() => [
    ...
    { key: "pan", header: "PAN", render: (r: Shareholder) => editing?._id === r._id ? <Input value={editing.pan} onChange={(e) => setEditing({ ...(editing as Shareholder), pan: e.target.value })} /> : r.pan },
    { key: "linkedDmatAccount", header: "DMAT", render: (r: Shareholder) => editing?._id === r._id ? (
      <Select value={(editing.linkedDmatAccount as any)?._id || (editing.linkedDmatAccount as any) || "none"} onValueChange={(v) => setEditing({ ...(editing as Shareholder), linkedDmatAccount: v === "none" ? undefined : v })}>
        ...
        <SelectContent>
          ...
          {(dmat ?? []).map((d) => <SelectItem key={d._id} value={d._id}>{d.accountNumber}</SelectItem>)}
        </SelectContent>
      </Select>
    ) : (typeof r.linkedDmatAccount === "object" && r.linkedDmatAccount ? (r.linkedDmatAccount as DmatAccount).accountNumber : "—") },
    { key: "actions", header: "Actions", render: (r: Shareholder) => editing?._id === r._id ? (
          ...
          <DialogContent>
            ...
            <div className="grid gap-3">
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="space-y-1"><Label>PAN</Label><Input value={form.pan} onChange={(e)=>setForm((f)=>({ ...f, pan: e.target.value }))} /></div>
